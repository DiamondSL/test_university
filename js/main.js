const asyncCallback = async ({callback = functionFallback({fallbackText: 'no callback provided', context: this})}) => callback instanceof Function && callback !== functionFallback ? await callback : callback
const functionFallback = ({fallbackText = String('No function provided'), context = this}) => console.warn({text: fallbackText, place: context});
...
class CSSProperty {
  constructor(propertyName, value) {
    if (Array.isArray(propertyName)) {
      return Object.fromEntries(propertyName.map((item, index) => {
...
const useAwaitDOMElementRender = (
  selector = 'body',
  callback = functionFallback({fallbackText: 'no callback provided', context: this}),
  observerConfig = new Object({childList: true, subtree: true}),
  disconnectCondition = true,
  disconnectOnRender = true,
  HTMLparentElement = document,
  logMutations = false
) => {
  ...
  }))) : observerConfig === undefined || observerConfig === null && {childList: true, subtree: true, attributes: true}
...
useAwaitDOMElementRender('body').then((result) => {
  if (result instanceof CSSStyleDeclaration) {
    return documentStyles = window.getComputedStyle(result)
  }
})
...
customElements.define('container-title-wrapper', class ElementsContainerTitleWrapper extends HTMLElement {
  constructor() {
    ...
    awaitHTMLElementAttributeChange(this, ["parentElement"]).then((result) => {
...
customElements.define('elements-container', class ElementsContainer extends HTMLDivElement {
  ...
  constructElementTitle(title) {
    if (typeof title === "string" && title.length > 0) {
      const titleWrapper = document.createElement('header', {is: 'container-title-wrapper'})
  ...
  constructElementDescription(description) {
    if (typeof description === "string" && description.length > 0) {
      const articleDescriptionWrapper = document.createElement('article', {is: 'container-description-wrapper'});
  ...
  getTemplates("./html_templates.html", document.body).then((result) => {
    ...
    customElements.define('newsletter-item',
      class NewsletterItem extends HTMLElement {
        constructor() {
          super()
        ...
        constructor() {
          ...
          this.addEventListener('click', (event) => {
            ...
            if (event.target === event.currentTarget) {
              return this.classList.contains('visible') ? this.hide() : this.show()
            }
          })
    ...
    customElements.define('newsletter-preview',
      class NewsletterPreview extends HTMLElement {
        constructImageFallback = (imageFallback, elementClasses, styles, ...args) => {
          ...
          if (!checkIfElementIsHTMLNode(imageFallback)) {
            ...
            if (visualItem instanceof HTMLVideoElement) {
            ...
            if (visualItem instanceof HTMLPictureElement) {
              ...
              Object.defineProperty(visualItemChildren.sourceElement,
                'src', {
  ...
  const awaitHTMLElementAttributeChange = (element, attributes, callback) => {
    ...
    const initialAttributeVales = Object.fromEntries(attributes.map((item) => {
      return [item, element.getAttribute(item) || element[item]]
    }))
    const mutationCallback = (mutationsList) => {
      if (attributes.length > 0) {
        for (const mutation of mutationsList) {
          attributes.some((item, index) => {
