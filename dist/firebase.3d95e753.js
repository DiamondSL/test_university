...
})({"b8PMv":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "fbc5fbce3d95e753";
"use strict";
...
},{}],"krU9j":[function(require,module,exports) {
...
const firebaseModalNewsletterItemAuthor = `<form class="styled-form" id="authorDetails" data-flex-column data-centered-item data-default-gap>
...
const firebaseModalNewsletterItem = `<form class="styled-form" id="newsletter-post-form" data-flex-row data-default-gap data-wrap>
...
const firebaseModalSubmit = `<div class="submit-container"><button type="submit" disabled id="modal-submit">New newsletter item</button></div>`;
const createdFormWrapper = ({ classList ="firebase-form-wrapper" , attributes =[
    {
        name: "data-flex-row",
        value: ""
    },
...
const renderAllPosts = ({ author , authorEmail , posts , container  })=>{
    posts.forEach((item)=>{
        const newsletterItem = templateNewsletterPreview({
            postName: item.name,
            authorName: author,
            postCategory: item.category,
            postMarkup: item.markup,
...
document.addEventListener("DOMContentLoaded", (event)=>{
    ...
    let validPost = false;
    let allPosts = null;
    (0, _firebaseInitialize.getAllPosts).then((result)=>result.toJSON()).then((result)=>allPosts = Object.entries(result).map((item)=>({
                author: item[0],
                properties: item[1]
            }))).then((result)=>{
        result.map((authorEntity)=>{
            const posts = authorEntity.properties.items && Object.values(authorEntity.properties.items);
            posts && renderAllPosts({
                author: authorEntity.author,
                posts: posts,
                container: postsContainer,
                authorEmail: authorEntity.properties.email
            });
        });
    ...
    newPostForm.addEventListener("change", (event)=>{
        ...
        if (validItem.valid) newPostData[inputName?.toString()?.trim()] = inputName === "postMarkup" ? inputValue?.toString() : inputValue?.toString()?.trim();
        if (userIsValid && Object.values(newPostData).every((item)=>item.length > 2)) {
            ...
            submitModalButton.addEventListener("click", (event)=>{
                (0, _firebaseInitialize.writePost)(newPostData).then(async (result)=>{
                    await result;
                });
            });
        }
    ...
    authorNameForm && authorNameForm.addEventListener("submit", (event)=>{
        event.preventDefault();
        stopPropagation(event);
        (0, _firebaseInitialize.getAuthor)(authorFormData.firstName, authorFormData.lastName).then((snapshot)=>{
            if (snapshot.exists()) {
                userIsValid = true;
            ...
            } else (0, _firebaseInitialize.setAuthor)(authorFormData.firstName, authorFormData.lastName, authorFormData.authorEmail).then(async (newAuthor)=>{
                return newAuthor;
            }).then(()=>{
                userIsValid = true;
            }).catch((error)=>{
    ...
    authorNameForm && authorNameForm.addEventListener("change", (event)=>{
        const { validity: validItem , value: inputValue , name: inputName  } = event.target;
        if (validItem.valid) authorFormData[inputName?.toString()?.trim()] = inputValue?.toString()?.trim();
        if (authorNameForm.children.length - 1 === Object.keys(authorFormData).length) authorNameForm.lastElementChild.click();
    });
    ...
    uploadForm && uploadForm.addEventListener("submit", (event)=>{
        stopPropagation(event);
        event.preventDefault();
        if (file && file instanceof File) (0, _firebaseInitialize.uploadVisualElement)(file).then((result)=>{
    ...
    return {
        dataBase: (0, _firebaseInitialize.dataBase),
        setAuthor: (0, _firebaseInitialize.setAuthor),
        writePost: (0, _firebaseInitialize.writePost)
    };
});

},{"./firebaseInitialize":"3z16D"}]},["b8PMv","krU9j"], "krU9j", "parcelRequire189f")

//# sourceMappingURL=firebase.3d95e753.js.map
